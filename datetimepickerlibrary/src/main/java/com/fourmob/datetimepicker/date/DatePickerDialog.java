...
public class DatePickerDialog extends DialogFragment implements
	...
	private void setCurrentView(int currentView, boolean forceRefresh) {
		try {
			...
			case MONTH_AND_DAY_VIEW:
				...
				break;
			case YEAR_VIEW:
				ObjectAnimator yearAnim = Utils.getPulseAnimator(mYearView,
						0.85F, 1.1F);
				if (mDelayAnimation) {
	...
	public void initialize(OnDateSetListener onDateSetListener, int year,
		...
		try {
			if (year > MAX_YEAR)
				throw new IllegalArgumentException("year end must < "
						+ MAX_YEAR);
			if (year < MIN_YEAR)
				throw new IllegalArgumentException("year end must > "
						+ MIN_YEAR);
	...
	public void onCreate(Bundle bundle) {
		try {
			...
			if (bundle != null) {
				...
				mCalendar
						.set(Calendar.MONTH, bundle.getInt(KEY_SELECTED_MONTH));
	...
	public View onCreateView(LayoutInflater layoutInflater, ViewGroup parent,
		...
		try {
			mDayOfWeekView = ((TextView) view
					.findViewById(R.id.date_picker_header));

			if (mDayOfWeekView != null) {
				mDayOfWeekView.setBackgroundColor(getResources().getColor(
			...
			mAnimator = ((AccessibleDateAnimator) view
	...
	public void onSaveInstanceState(Bundle bundle) {
		try {
			...
			bundle.putInt(KEY_SELECTED_DAY,
					mCalendar.get(Calendar.DAY_OF_MONTH));

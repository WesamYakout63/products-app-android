...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	protected void closeWithouAnimation(int position) {
		try {
			closeWithouAnimation(swipeListView.getChildAt(position - swipeListView.getFirstVisiblePosition()).findViewById(swipeFrontView), position);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void closeWithouAnimation(View view, final int position) {
		if (opened.get(position)) {
			int moveTo = 0;
			...
			animate(view).translationX(moveTo).setDuration(0).setListener(new AnimatorListenerAdapter() {
				@Override
	...
	public AbsListView.OnScrollListener makeScrollListener() {
		return new AbsListView.OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				...
				// }
				// } else {
				// boolean onLastItemList = firstVisibleItem + visibleItemCount
				// >= totalItemCount;
				// if (onLastItemList) {
				// isLastItem = true;
	...
	void closeOpenedItemsWithoutAnim(int position) {
		try {
			if (opened != null) {
				int start = swipeListView.getFirstVisiblePosition();
				int end = swipeListView.getLastVisiblePosition();
				for (int i = start; i <= end; i++) {
					if (opened.get(i) && i != position) {
						closeWithouAnimation(swipeListView.getChildAt(i - start).findViewById(swipeFrontView), i);
					}
				}
			}
		} catch (Exception e) {
